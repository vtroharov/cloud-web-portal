...
import { getVMsPerTenant } from '../../actions/vms';
...
const Dashboard = ({ auth: { isAuthenticated, user, loading }, getVMsPerTenant, vms: { vms } }) => {
  useEffect(() => {
    getVMsPerTenant(user.permissions);
  }, [getVMsPerTenant, user.permissions]);
  ...
  return (loading && user.permissions === null ? <Spinner /> : 
    <Paper component='div' className={classes.paperFull}>
      {isOpen1 && <PopUp 
        content={<>
          <h1>CPU Allocated</h1>
          <DashboardGraph1 cat={"vm"} showLabel={""} display={false} valueX={"100%"} valueY={(vms.length * 50).toString() + "px"} />
          </>}
      ...
      {isOpen2 && <PopUp 
        content={<>
          ...
          <DashboardGraph2 cat={"vm"} showLabel={""} display={false} valueX={"100%"} valueY={(vms.length * 50).toString() + "px"} />
          </>}
      ...
      {isOpen3 && <PopUp 
        content={<>
          <h1>Uptime (hours)</h1>
          <DashboardGraph3 cat={"vm"} showLabel={""} display={false} valueX={"100%"} valueY={(vms.length * 50).toString() + "px"} />
          </>}
      ...
      {isOpen4 && <PopUp 
        content={<>
          <h1>Space (GB)</h1>
          <DashboardGraph4 cat={"vm"} showLabel={"{name}"} display={false} valueX={"100%"} valueY={(vms.length * 50).toString() + "px"} />
          </>}
            ...
            <Grid item xs={12}>
              <Grid container justify="flex-start" spacing={2}>
                ...
                <Grid item className={classes.graphBorder} onClick={togglePopup1}>
                  CPU Allocated
                  <DashboardGraph1 cat={"vm"} showLabel={"{vm}"} display={true} valueX={"50vh"} valueY={"100%"} />
                </Grid>
                <Grid item className={classes.graphBorder} onClick={togglePopup2}>
                  RAM Allocated (GB)
                  <DashboardGraph2 cat={"vm"} showLabel={"{vm}"} display={true} valueX={"50vh"} valueY={"100%"} />
                </Grid>
              </Grid>
            </Grid>
            <Grid item xs={12}>
              <Grid container justify="flex-start" spacing={2}>
                ...
                <Grid item className={classes.graphBorder} onClick={togglePopup3}>
                  Uptime (hours)
                  <DashboardGraph3 cat={"vm"} showLabel={"{vm}"} display={true} valueX={"50vh"} valueY={"100%"} />
                </Grid>
                <Grid item className={classes.graphBorder} onClick={togglePopup4}>
                  Space (GB)
                  <DashboardGraph4 cat={"vm"} showLabel={"{name}"} display={true} valueX={"50vh"} valueY={"100%"} />
                </Grid>
              </Grid>
            </Grid>
          </Grid>
...
Dashboard.propTypes = {
  ...
  getVMsPerTenant: PropTypes.func.isRequired,
};
...
export default connect(mapStateToProps, { getVMsPerTenant })(Dashboard);
